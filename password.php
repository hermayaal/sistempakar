...
if (isset($_POST['save'])) {
    ...
    if (empty($error)) {
        if (mysqli_num_rows(mysqli_query($con, "select * from admin where id_admin='" . $id_admin . "' and password='" . $password . "'")) > 0) {
            mysqli_query($con, "update admin set password='" . $password_baru . "' where id_admin='" . $id_admin . "'");
